pkgs: self: {
  ...
  pyproject =
    ...
    then self.callPackage ./cuda/pyproject.nix {}
    else if self.platform == "rocm"
    then self.callPackage ./rocm/pyproject.nix {}
    else throw "Unsupported platform ${self.platform}";
  ...
  call-poetry =
    ...
    )
    {
      lockfile =
        if self.platform == "cuda"
        then "environment/cuda/poetry.lock"
        else if self.platform == "rocm"
        then "environment/rocm/poetry.lock"
        else throw "Unsupported platform ${self.platform}";
    };

  poetrylock =
    if self.platform == "cuda"
    then ./cuda/poetry.lock
    else if self.platform == "rocm"
    then ./rocm/poetry.lock
    else throw "Unsupported platform ${self.platform}";
  ...
  platformOverlay =
    if self.platform == "cuda"
    then self.callPackage ./cuda/overlay.nix {}
    else if self.platform == "rocm"
    then self.callPackage ./rocm/overlay.nix {}
    else throw "Unsupported platform ${self.platform}";
